--MIG_CHARGE_OFF_ACTIVITY_D
--288

WITH CTE AS(
SELECT DISTINCT
CHARGE.ARRANGEMENT, CASE WHEN CHARGE.ARRANGEMENT = RTRIM(LTRIM('LA'+CREDIT.CANUCR)) THEN RTRIM(LTRIM('LA'+CREDIT.CANUCR)) 
...
CHARGE.TXN_AMOUNT, CAD.DSSALD AS TXN_AMOUNT_AUDITORIA,
CHARGE.CURRENCY, 
...
FROM MIG_CHARGE_OFF_ACTIVITY_D CHARGE
LEFT JOIN CACREDIT CREDIT
ON RTRIM(LTRIM(RIGHT(CHARGE.ARRANGEMENT,7))) = RTRIM(LTRIM(CREDIT.CANUCR))
LEFT JOIN (SELECT DISTINCT * FROM MIG_HOM_SUCURSAL) SUC
ON CREDIT.CAAGEN = SUC.CODIGO
AND CHARGE.UPLOAD_COMPANY = SUC.COMPANY
LEFT JOIN (SELECT DISTINCT CANUCR,CAAETR,CAMETR,CADETR,CACAPI FROM  CAHISPTM) CAH ON RTRIM(LTRIM(CREDIT.CANUCR)) = RTRIM(LTRIM(CAH.CANUCR))
...
INNER JOIN CADISCAP CAD ON RTRIM(LTRIM(CREDIT.CANUCR))=RTRIM(LTRIM(CAD.CANUCR))
AND CHARGE.TXN_AMOUNT = CAD.DSSALD 
)
